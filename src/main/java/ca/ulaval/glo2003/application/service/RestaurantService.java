...
public class RestaurantService {
    ...
    public void updateRestaurant(String ownerId, String restaurantId, UpdateRestaurantDto updateRestaurantDto) throws Exception {

        Restaurant oldRestaurant = restaurantRepository.getOwnerRestaurantById(ownerId, restaurantId);
        LocalDate actualDate = LocalDate.now();
        List<Reservation> reservations = restaurantRepository.getReservationsByRestaurantId(ownerId, restaurantId).stream()
                .filter(reservation -> reservation.getDate().isAfter(actualDate)).toList();
    ...
    private void removeReservationBeforeUpdate(String restaurantId, List<Reservation> reservations, LocalDate actualDate) throws NotFoundException {
    ...
    private void addReservations(List<Reservation> reservationToAdd, Restaurant updatedRestaurant) throws NotFoundException, InvalidParameterException {
    ...
    private static List<Reservation> getValidReservations(List<Reservation> reservations, Restaurant updatedRestaurant) {
        reservations.stream().peek(reservation -> reservation.setEndTime(Util.calculEndTime(reservation.getStartTime(), updatedRestaurant.getReservation().duration())));

        reservations = reservations.stream()
                .sorted(Comparator.comparing(Reservation::getDate))
                .toList();

        reservations = reservations.stream().filter(reservation -> reservation.getStartTime().isBefore(updatedRestaurant.getHours().getClose()) &&
                                                     !reservation.getEndTime().isAfter(updatedRestaurant.getHours().getClose()) &&
                                                     !reservation.getStartTime().isBefore(updatedRestaurant.getHours().getOpen())).toList();
    ...
    private  Restaurant createUpdateRestaurant(UpdateRestaurantDto updateRestaurantDto, Restaurant oldRestaurant) {
        Restaurant updatedRestaurant = new Restaurant(oldRestaurant);

        Hours hours = updateRestaurantDto.hours() == null ? oldRestaurant.getHours() : new Hours(updateRestaurantDto.hours().getOpen() == null ? oldRestaurant.getHours().getOpen() : updateRestaurantDto.hours().getOpen(), updateRestaurantDto.hours().getClose() == null ? oldRestaurant.getHours().getClose() : updateRestaurantDto.hours().getClose());
        updatedRestaurant.setHours(hours);
        updatedRestaurant.setName(updateRestaurantDto.name() == null? oldRestaurant.getName() : updateRestaurantDto.name());
        updatedRestaurant.setCapacity(updateRestaurantDto.capacity() == 0? oldRestaurant.getCapacity() : updateRestaurantDto.capacity());
        updatedRestaurant.setDuration(updateRestaurantDto.reservations()==null? oldRestaurant.getReservation().duration() : updateRestaurantDto.reservations().duration()==0? oldRestaurant.getReservation().duration() : updateRestaurantDto.reservations().duration());
